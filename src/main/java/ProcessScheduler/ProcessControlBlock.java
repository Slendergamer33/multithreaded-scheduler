...
public class ProcessControlBlock {
	 ...
	 * then you can calculate the execution time of PCB.
	 * this method is called when PCB execution is finished.
	 */
	public void setExecutionTime(long execTime) {
		if (arrivalTime == -1) {
			throw new IllegalStateException("Couldn't set execution time. Arrival time was never recorded");
		}
		//getting the current system time
		long currentTime = System.nanoTime();

		//converted ns to ms
		this.executionTime = (currentTime - arrivalTime) / 1_000_000; // Convert to ms
	}

	public String getContextSwitchCount() {
		return "";
	}
